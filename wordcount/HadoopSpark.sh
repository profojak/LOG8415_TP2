...
for input in input/*.txt; do
        ...
        for i in {1..3}; do
		echo -n "Spark: "
		SPARK_TIME=$(time (spark-submit spark.py $input wordcount/output &> /dev/null) )
                times_spark+=($SPARK_TIME)

		hdfs dfs -rm -r wordcount/output &> /dev/null

		echo -n "Hadoop: "
                HADOOP_TIME=$(time (hadoop jar /usr/local/hadoop-3.3.1/share/hadoop/tools/lib/hadoop-streaming-3.3.1.jar -mapper wordcount/mapper.py -reducer wordcount/reducer.py -input wordcount/input/$input -output wordcount/output &> /dev/null) )
